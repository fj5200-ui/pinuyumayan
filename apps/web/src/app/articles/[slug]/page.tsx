      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <article ref={articleRef} className="bg-white dark:bg-stone-800 rounded-2xl shadow-sm p-8 border border-stone-100 dark:border-stone-700">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-stone-400">約 {readingTime} 分鐘閱讀</span>
          </div>
          ...
          <div className="mt-8 bg-white dark:bg-stone-800 rounded-2xl shadow-sm p-6 border dark:border-stone-700">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {related.slice(0, 3).map((r: any) => (
                <Link key={r.id} href={`/articles/${r.slug}`} className="p-4 bg-stone-50 dark:bg-stone-700/50 rounded-xl hover:bg-stone-100 dark:hover:bg-stone-700 transition">
                  <span className="text-xs bg-amber-100 dark:bg-amber-900/50 text-amber-700 px-2 py-0.5 rounded-full">{r.category}</span>
                  <h3 className="font-medium mt-2 dark:text-stone-200 line-clamp-2 text-sm">{r.title}</h3>
                </Link>
              ))}
            </div>
          </div>
          ...
          {user ? (
            <div className="flex gap-3 mb-6">
              <input value={newComment} onChange={e => setNewComment(e.target.value)} onKeyDown={e => e.key === "Enter" && submitComment()}
                placeholder="分享你的想法..." className="flex-1 px-4 py-2.5 rounded-xl border dark:border-stone-600 dark:bg-stone-700 dark:text-stone-100 focus:border-amber-500 outline-none" />
