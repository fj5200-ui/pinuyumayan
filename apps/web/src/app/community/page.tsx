    ...
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      <div className="md:hidden mb-4">
        <button onClick={() => setShowMobileBoards(!showMobileBoards)}
          className="w-full flex items-center justify-between bg-white dark:bg-[#1a1a1a] border dark:border-[#333] rounded-[var(--radius-md)] px-4 py-3">
          <span className="flex items-center gap-2">
            <span>{currentBoard.icon}</span>
            <span className="font-medium dark:text-gray-100">{currentBoard.name}</span>
          </span>
          <span className="text-[var(--text-light)]">{showMobileBoards ? "▲" : "▼"}</span>
        </button>
        {showMobileBoards && (
          <div className="mt-2 bg-white dark:bg-[#1a1a1a] border dark:border-[#333] rounded-[var(--radius-md)] p-2 space-y-1">
            {BOARDS.map(b => (
              <button key={b.id} onClick={() => { setBoard(b.id); setSelectedPost(null); setShowMobileBoards(false); }}
                className={`w-full text-left p-3 rounded-lg text-sm ${board === b.id ? "bg-[rgba(153,27,27,0.06)] dark:bg-[#222]/30 text-[var(--red)] dark:text-[var(--yellow)]" : "text-[var(--text-soft)] dark:text-[var(--text-light)]"}`}>
                {b.icon} {b.name}
              </button>
            ))}
          </div>
      ...
      <div className="grid md:grid-cols-4 gap-6">
        ...
        <aside className="hidden md:block md:col-span-1">
          <div className="bg-white dark:bg-[#1a1a1a] rounded-[var(--radius-md)] border dark:border-[#333] p-4 sticky top-20">
            <h3 className="font-bold text-sm text-[var(--text-soft)] dark:text-[var(--text-light)] mb-3">討論板</h3>
            {BOARDS.map(b => (
              <button key={b.id} onClick={() => { setBoard(b.id); setSelectedPost(null); }}
                className={`w-full text-left p-3 rounded-lg mb-1 transition text-sm ${board === b.id ? "bg-[rgba(153,27,27,0.06)] dark:bg-[#222]/30 text-[var(--red)] dark:text-[var(--yellow)] font-medium" : "hover:bg-[var(--cream)] dark:hover:bg-[#333]/50 text-[var(--text-soft)] dark:text-[var(--text-light)]"}`}>
                <span className="mr-2">{b.icon}</span>{b.name}
                <p className="text-xs text-[var(--text-light)] mt-0.5 ml-6">{b.desc}</p>
              </button>
            ))}

            {/* Stats */}
            <div className="mt-4 pt-4 border-t dark:border-[#333] space-y-2 text-xs text-[var(--text-light)]">
              <div className="flex justify-between"><span>當前版面</span><span className="font-medium text-[var(--text-soft)] dark:text-[var(--text-light)]">{totalPosts} 篇</span></div>
            </div>
        ...
        <div className="md:col-span-3">
          {/* Board header */}
          <div className={`bg-gradient-to-r ${currentBoard.color} rounded-[var(--radius-md)] p-5 mb-4 text-white`}>
          ...
          {showNewPost && (
            <div className="bg-white dark:bg-[#1a1a1a] rounded-[var(--radius-md)] border dark:border-[#333] p-6 mb-4">
              ...
              <textarea value={content} onChange={e => setContent(e.target.value)} placeholder="寫下你的想法..." rows={5}
                className="w-full px-4 py-2.5 border rounded-[var(--radius-md)] dark:border-[#444] dark:bg-[#222] dark:text-gray-100 mb-3 focus:border-red-500 outline-none transition resize-none" />
              <div className="flex gap-2 justify-end">
                <button onClick={() => { setShowNewPost(false); setTitle(""); setContent(""); }}
                  className="px-5 py-2 rounded-lg border dark:border-[#444] dark:text-[var(--text-light)] text-sm hover:bg-[var(--cream)] dark:hover:bg-[#333] transition">取消</button>
                <button onClick={submitPost} className="bg-[var(--red)] text-white px-6 py-2 rounded-lg hover:bg-[var(--red)] text-sm font-medium transition">發布貼文</button>
              </div>
            </div>
          )}
          ...
          {selectedPost ? (
            <div className="bg-white dark:bg-[#1a1a1a] rounded-[var(--radius-md)] border dark:border-[#333] overflow-hidden">
              <div className="p-6">
                <button onClick={() => setSelectedPost(null)} className="text-sm text-[var(--red)] dark:text-[var(--yellow)] hover:underline mb-4 flex items-center gap-1">← 返回列表</button>
                <h2 className="text-2xl font-bold dark:text-gray-100 mb-3">{selectedPost.title}</h2>
                <div className="flex items-center gap-3 text-sm text-[var(--text-light)] mb-4 flex-wrap">
                  <span className="flex items-center gap-1.5">
                    <span className="w-7 h-7 bg-gradient-to-br from-[rgba(217,119,6,0.1)] to-[var(--yellow)] rounded-full flex items-center justify-center text-xs text-white font-bold">{(selectedPost.authorName || '?')[0]}</span>
                    <span className="text-[var(--text-soft)] dark:text-[var(--text-light)] font-medium">{selectedPost.authorName}</span>
                  </span>
                    ...
                    <div className="space-y-3 mb-5">
                      {selectedPost.replies.map((r: any, i: number) => (
                        <div key={r.id || i} className="flex gap-3">
                          <div className="w-8 h-8 bg-gradient-to-br from-stone-200 to-stone-300 dark:from-stone-600 dark:to-stone-700 rounded-full flex items-center justify-center text-xs font-bold text-[var(--text-soft)] dark:text-[var(--text-light)] flex-shrink-0">
                            {(r.authorName || '?')[0]}
                          </div>
                          <div className="flex-1 bg-[var(--cream)] dark:bg-[#222]/50 rounded-[var(--radius-md)] p-3">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-medium text-sm text-[var(--text-main)] dark:text-gray-200">{r.authorName}</span>
                              <span className="text-xs text-[var(--text-light)]">{timeAgo(r.createdAt)}</span>
                            </div>
                            <p className="text-[var(--text-soft)] dark:text-[var(--text-light)] text-sm leading-relaxed">{r.content}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : <p className="text-[var(--text-light)] text-sm mb-5">暫無回覆，成為第一個回覆的人！</p>}

                  {user ? (
                    <div className="flex gap-2">
                      <div className="w-8 h-8 bg-gradient-to-br from-[rgba(217,119,6,0.1)] to-[var(--yellow)] rounded-full flex items-center justify-center text-xs text-white font-bold flex-shrink-0">
                        {(user.name || '?')[0]}
                      </div>
                      <input value={replyText} onChange={e => setReplyText(e.target.value)} onKeyDown={e => e.key === "Enter" && submitReply()}
                        placeholder="輸入回覆..." className="flex-1 px-4 py-2.5 border rounded-[var(--radius-md)] dark:border-[#444] dark:bg-[#222] dark:text-gray-100 text-sm focus:border-red-500 outline-none transition" />
                      <button onClick={submitReply} className="bg-[var(--red)] text-white px-5 py-2.5 rounded-[var(--radius-md)] hover:bg-[var(--red)] text-sm font-medium transition">送出</button>
                    </div>
                  ) : <p className="text-sm text-[var(--text-light)]"><Link href="/login" className="text-[var(--red)] dark:text-[var(--yellow)] hover:underline">登入</Link>後即可回覆</p>}
                </div>
              </div>
            </div>
          ) : (
            ...
            <>
              ...
              {!loading && posts.length > 0 && (
                <div className="flex items-center justify-between mb-3">
                  <p className="text-sm text-[var(--text-light)]">{totalPosts} 篇貼文</p>
              ...
              {loading ? (
                <div className="space-y-3">
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className="bg-white dark:bg-[#1a1a1a] rounded-[var(--radius-md)] border dark:border-[#333] p-5 animate-pulse">
                      <div className="h-5 w-2/3 bg-gray-200 dark:bg-[#222] rounded mb-3" />
                      <div className="h-4 w-full bg-gray-200 dark:bg-[#222] rounded mb-2" />
                ...
                <div className="space-y-3">
                  {sortedPosts.map(p => (
                    <div key={p.id} onClick={() => openPost(p.id)}
                      className="bg-white dark:bg-[#1a1a1a] rounded-[var(--radius-md)] border dark:border-[#333] p-5 hover:shadow-sm hover:border-amber-200 dark:hover:border-amber-800 transition cursor-pointer group">
                      <div className="flex items-start gap-4">
                        <div className="w-10 h-10 bg-gradient-to-br from-[rgba(217,119,6,0.1)] to-[var(--yellow)] dark:from-[var(--red)] dark:to-orange-800 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                          {(p.authorName || '?')[0]}
                        </div>
                        <div className="flex-1 min-w-0">
                          <h3 className="font-bold text-lg dark:text-gray-100 mb-1 group-hover:text-[var(--red)] dark:group-hover:text-[var(--yellow)] transition truncate">{p.title}</h3>
                          <p className="text-[var(--text-soft)] dark:text-[var(--text-light)] text-sm line-clamp-2 leading-relaxed">{p.content}</p>
              ...
              {user && !showNewPost && (
                <button onClick={() => setShowNewPost(true)}
                  className="md:hidden fixed bottom-6 right-6 w-14 h-14 bg-[var(--red)] text-white rounded-full shadow-lg flex items-center justify-center text-2xl hover:bg-[var(--red)] transition z-40">
                  ✏️
                </button>
              )}
            </>
