...
export default function Home() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get<any>("/api/admin/site-settings").catch(() => ({ settings: null })),
    ]).then(([t, a, e, v, d, cs, lb, ss]) => {
      ...
      setStats({ tribes: tList.length || 8, vocab: vList.length || 15, articles: aList.length || 6, events: eList.length || 6 });

      // Site settings
      const settings = ss?.settings;
      if (settings?.heroSlides?.length) setHeroSlides(settings.heroSlides);
      if (settings?.homeSections?.length) setHomeSections(settings.homeSections);
  ...
  useEffect(() => {
    ...
    heroTimer.current = setInterval(() => setHeroIdx(prev => (prev + 1) % heroSlides.length), 5000);
  ...
  const isSectionEnabled = (id: string) => {
    if (!homeSections.length) return true; // default: all enabled
    const sec = homeSections.find(s => s.id === id);
  ...
  const sections: Record<string, () => React.ReactNode> = {
      ...
      <section className="section" style={{ background: "var(--cream)" }}>
        <div className="w-[min(1180px,92%)] mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div className="section-heading" style={{ marginBottom: 0 }}><h2>卑南八社</h2><p>認識卑南族各部落的歷史與文化</p></div>
            <Link href="/tribes" className="text-sm font-bold hover:text-[var(--red)] transition" style={{ color: "var(--text-soft)" }}>查看全部 →</Link>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            {tribes.map((t: any) => (
              <Link key={t.id} href={`/tribes/${t.id}`} className="card-solid group">
                ...
                {t.population && <div className="flex items-center gap-1 mt-3 text-xs" style={{ color: "var(--text-light)" }}>人口約 {t.population?.toLocaleString()} 人</div>}
              </Link>
            ))}
          </div>
        </div>
    ...
    sites: () => sites.length > 0 ? (
      <section className="section bg-white dark:bg-[#1a1a1a]">
        <div className="w-[min(1180px,92%)] mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div className="section-heading" style={{ marginBottom: 0 }}><h2>文化景點</h2><p>探訪卑南族重要的文化遺址與祭典場域</p></div>
            <Link href="/cultural-sites" className="text-sm font-bold hover:text-[var(--red)] transition" style={{ color: "var(--text-soft)" }}>查看全部 →</Link>
          </div>
              ...
              <div key={s.id} className="card-solid group">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2"><h3 className="font-bold truncate">{s.name}</h3><span className="tag-red shrink-0">{s.type}</span></div>
    ...
    articles: () => (
      <section className="section bg-white dark:bg-[#1a1a1a]">
        <div className="w-[min(1180px,92%)] mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div className="section-heading" style={{ marginBottom: 0 }}><h2>文化誌</h2><p>深入了解卑南族文化的各個面向</p></div>
            <Link href="/articles" className="text-sm font-bold hover:text-[var(--red)] transition" style={{ color: "var(--text-soft)" }}>查看全部 →</Link>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {articles.map((a: any) => (
              <Link key={a.id} href={`/articles/${a.slug}`} className="card-solid group overflow-hidden">
                <div className="flex items-center gap-2 mb-3"><span className="tag-yellow">{a.category}</span></div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              <div className="flex items-center justify-between mb-6">
                <div className="section-heading" style={{ marginBottom: 0 }}><h2>族語學習</h2><p>一起來學卑南語</p></div>
            ...
            <div>
              <div className="section-heading mb-6"><h2 className="!text-xl">學習排行榜</h2></div>
    ...
    events: () => (
      <section className="section bg-white dark:bg-[#1a1a1a]">
        <div className="w-[min(1180px,92%)] mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div className="section-heading" style={{ marginBottom: 0 }}><h2>活動祭典</h2><p>即將舉行的文化活動</p></div>
            <Link href="/events" className="text-sm font-bold hover:text-[var(--red)] transition" style={{ color: "var(--text-soft)" }}>查看全部 →</Link>
          </div>
          {events.length > 0 ? (
            <div className="grid md:grid-cols-3 gap-5">
              {events.map((e: any) => {
                ...
                return (
                  <div key={e.id} className="card-solid group">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      {isUpcoming && <span className="tag-green flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full animate-pulse-slow" style={{ background: "var(--green)" }} />即將到來</span>}
                    </div>
    ...
    cta: () => (
      <section className="py-16" style={{ background: "var(--cream)" }}>
        <div className="banner-dark w-[min(1180px,92%)] mx-auto">
          ...
          <p className="text-white/70 text-lg mb-8 max-w-2xl mx-auto">一起記錄、分享、傳承卑南族珍貴的文化遺產。每個人的參與都是文化延續的重要力量。</p>
  ...
  /* Build section order */
  const defaultOrder = ["hero", "daily", "stats", "tribes", "sites", "articles", "vocab", "events", "cta"];
  const sectionOrder = homeSections.length > 0
    ? homeSections.filter(s => s.enabled).map(s => s.id)
    : defaultOrder;
  ...
  return (
    <>
      ...
      {sectionOrder.map((id, i) => {
        ...
        // Show color bar right after hero
        if (id === "hero") return (
          <div key={id}>
            {renderer()}
        ...
        if (loading && ["tribes", "sites", "articles", "vocab", "events"].includes(id)) {
          return (
        ...
        return <div key={id}>{renderer()}</div>;
      })}
    </>
  );
}
