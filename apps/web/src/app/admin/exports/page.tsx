...
const EXPORT_TYPES = [
  ...
  { id: "vocabulary", name: "詞彙", icon: "📖", description: "匯出卑南語詞彙庫（族語、中文、英文、分類、發音）", color: "bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800" },
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold dark:text-stone-100">📊 資料匯出</h1>
          <p className="text-stone-500 dark:text-stone-400 mt-1">將平台資料匯出為 CSV 格式</p>
        </div>
        <button onClick={handleExportAll} disabled={!!downloading}
          className="px-4 py-2 bg-amber-700 text-white rounded-lg text-sm hover:bg-amber-800 transition disabled:opacity-50">
          📦 全部匯出
        </button>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        {EXPORT_TYPES.map(t => (
          <div key={t.id} className={`rounded-xl p-6 border transition hover:shadow-md ${t.color}`}>
            <div className="text-3xl mb-3">{t.icon}</div>
            <h3 className="font-bold text-lg dark:text-stone-100 mb-1">{t.name}</h3>
            <p className="text-stone-500 dark:text-stone-400 text-sm mb-4">{t.description}</p>
            <button onClick={() => handleExport(t.id)} disabled={downloading === t.id}
              className="w-full px-4 py-2.5 bg-white dark:bg-stone-800 border dark:border-stone-600 rounded-lg text-sm font-medium hover:bg-stone-50 dark:hover:bg-stone-700 transition disabled:opacity-50 dark:text-stone-200">
      ...
      {recentExports.length > 0 && (
        <div className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 p-6">
          <h3 className="font-bold dark:text-stone-100 mb-4">📜 本次匯出紀錄</h3>
          <div className="space-y-2">
            {recentExports.map((exp, i) => {
              const info = EXPORT_TYPES.find(t => t.id === exp.type);
              return (
                <div key={i} className="flex items-center justify-between py-2 border-b dark:border-stone-700 last:border-0">
                  <div className="flex items-center gap-2">
                    <span>{info?.icon || "📄"}</span>
                    <span className="text-sm dark:text-stone-200">{info?.name || exp.type}</span>
                  </div>
      ...
      {/* Info */}
      <div className="mt-8 bg-stone-50 dark:bg-stone-800/50 rounded-xl p-6 border dark:border-stone-700">
        <h3 className="font-bold dark:text-stone-100 mb-2">ℹ️ 匯出說明</h3>
        <ul className="text-sm text-stone-500 dark:text-stone-400 space-y-1">
