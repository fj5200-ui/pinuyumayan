...
const TYPE_ICONS: Record<string, string> = { translation: "", summary: "", recommendation: "", moderation: "", analytics: "", other: "" };
          ...
          {agents.map(a => (
            <div key={a.id} className="bg-white dark:bg-[#1a1a1a] rounded-[var(--radius-md)] border dark:border-[#333] p-5">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => openEditor(a)} className="flex-1 text-xs text-center py-1.5 rounded-lg text-[var(--red)] border border-amber-200 dark:border-amber-800 hover:bg-white dark:hover:bg-[#222]/20 transition">編輯</button>
                <button onClick={() => del(a.id)} className="text-xs px-3 py-1.5 rounded-lg text-red-500 border border-red-200 dark:border-red-800 hover:bg-red-50 dark:hover:bg-red-900/20 transition"></button>
              </div>
            </div>
          ))}
        </div>
