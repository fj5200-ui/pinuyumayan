...
export default function AIToolsPage() {
  ...
  const statusColor = (s: string) => {
    ...
    return "bg-stone-100 text-stone-500 dark:bg-stone-700 dark:text-stone-400";
  };

  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold dark:text-stone-100">🤖 AI 工具管理</h1>
        <p className="text-sm text-stone-500 mt-1">管理與測試 AI 自動化功能</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 p-4 text-center">
          ...
          <p className="text-xs text-stone-500">啟用中</p>
        </div>
        <div className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 p-4 text-center">
          <p className="text-2xl font-bold text-purple-600">{tools.filter(t => t.status === "beta").length}</p>
          <p className="text-xs text-stone-500">Beta 測試</p>
        </div>
        <div className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 p-4 text-center">
          <p className="text-2xl font-bold text-amber-600">{tools.reduce((a, t) => a + t.usage, 0)}</p>
          <p className="text-xs text-stone-500">總呼叫次數</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <h3 className="font-bold text-sm text-stone-500 dark:text-stone-400 mb-2">工具列表</h3>
          {tools.map(t => (
            <div key={t.id}
              onClick={() => { setSelectedTool(t); setTestOutput(""); setTestInput(""); }}
              className={`bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 p-4 cursor-pointer transition hover:shadow-sm ${selectedTool?.id === t.id ? "ring-2 ring-amber-500" : ""}`}>
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-stone-400 mt-1">{t.usage} 次</p>
                </div>
              </div>
            </div>
          ))}
        ...
        {/* Test Panel */}
        <div className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 p-6 sticky top-20">
          {selectedTool ? (
            <>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-lg dark:text-stone-100">{selectedTool.name}</h3>
                    <p className="text-sm text-stone-500">{selectedTool.description}</p>
                  </div>
                </div>
              ...
              <div className="border-t dark:border-stone-700 pt-4">
                <h4 className="font-medium text-sm dark:text-stone-200 mb-2">測試面板</h4>
                <textarea value={testInput} onChange={e => setTestInput(e.target.value)} placeholder="輸入測試內容..."
                  rows={3} className="w-full px-3 py-2 border rounded-lg dark:border-stone-600 dark:bg-stone-700 dark:text-stone-100 text-sm mb-3" />
                <button onClick={runTest} disabled={testing}
                  className="bg-amber-700 text-white px-4 py-2 rounded-lg hover:bg-amber-800 transition text-sm disabled:opacity-50 w-full">
                  {testing ? "處理中..." : "執行測試"}
                </button>

                {testOutput && (
                  <div className="mt-4 bg-stone-50 dark:bg-stone-700/50 rounded-lg p-4">
                    <h5 className="text-xs font-medium text-stone-500 dark:text-stone-400 mb-2">輸出結果</h5>
                    <pre className="text-sm whitespace-pre-wrap dark:text-stone-200">{testOutput}</pre>
                  </div>
                )}
              </div>

              <div className="border-t dark:border-stone-700 pt-4 mt-4 grid grid-cols-2 gap-3 text-sm">
                <div className="text-stone-500">最後使用</div><div className="dark:text-stone-200">{selectedTool.lastUsed}</div>
                <div className="text-stone-500">呼叫次數</div><div className="dark:text-stone-200">{selectedTool.usage}</div>
                <div className="text-stone-500">狀態</div><div><span className={`text-xs px-2 py-0.5 rounded-full ${statusColor(selectedTool.status)}`}>{selectedTool.status}</span></div>
              </div>
            </>
          ) : (
