...
const TYPE_LABELS: Record<string, { label: string; icon: string; color: string }> = {
  ...
  event: { label: "活動", icon: "🎉", color: "bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300" },
};
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold dark:text-stone-100">📋 審核管理</h1>
          <p className="text-stone-500 dark:text-stone-400 mt-1">管理內容審核隊列</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
          className="px-4 py-2 border rounded-lg text-sm dark:bg-stone-800 dark:border-stone-700 dark:text-stone-200">
        ...
        <button onClick={loadQueue} className="px-4 py-2 bg-amber-700 text-white rounded-lg text-sm hover:bg-amber-800 transition">
          🔄 重新整理
        </button>
      </div>
      ...
      {loading ? <div className="text-center py-10 text-stone-400">載入中...</div> :
        items.length === 0 ? (
          <div className="text-center py-16 bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700">
            ...
            <p className="text-stone-500">目前沒有待審項目</p>
          </div>
        ) : (
          <div className="space-y-3">
            {items.map(item => {
              const typeInfo = TYPE_LABELS[item.type] || { label: item.type, icon: "📄", color: "bg-stone-100 text-stone-600" };
              const statusInfo = STATUS_LABELS[item.status] || { label: item.status, color: "bg-stone-100 text-stone-600" };
              const isReviewing = reviewingId === item.id;

              return (
                <div key={item.id} className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 p-5">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <h3 className="font-bold dark:text-stone-100 mb-1">{item.title}</h3>
                      <p className="text-stone-600 dark:text-stone-400 text-sm line-clamp-2">{item.content}</p>
                    ...
                    {item.status === "pending" && (
                      <div className="flex gap-2 ml-4">
                        {!isReviewing ? (
                          <button onClick={() => setReviewingId(item.id)} className="text-xs px-3 py-1.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-lg hover:bg-amber-200 transition">
                            審核
                          </button>
                        ) : (
                          <div className="flex flex-col gap-2 min-w-[200px]">
                            <input value={reviewNote} onChange={e => setReviewNote(e.target.value)} placeholder="審核備註（選填）"
                              className="px-3 py-1.5 border rounded-lg text-xs dark:bg-stone-700 dark:border-stone-600 dark:text-stone-200" />
                            <div className="flex gap-1">
                              ...
                              <button onClick={() => { setReviewingId(null); setReviewNote(""); }} className="text-xs px-2 py-1.5 bg-stone-200 dark:bg-stone-700 rounded-lg">取消</button>
                            </div>
                          </div>
                        )}
                      </div>
