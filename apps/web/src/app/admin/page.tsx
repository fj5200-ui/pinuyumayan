...
export default function AdminDashboard() {
  ...
  if (loading) return (
    <div className="space-y-6 animate-pulse">
      <div className="h-10 bg-stone-200 dark:bg-stone-700 rounded-lg w-64" />
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[...Array(8)].map((_, i) => <div key={i} className="h-24 bg-stone-200 dark:bg-stone-700 rounded-xl" />)}
      </div>
  ...
  const statCards = [
    ...
    { label: "部落數", value: dash.stats.tribes, icon: "🏘️", color: "from-amber-500 to-amber-600", bg: "bg-amber-50 dark:bg-amber-900/20", text: "text-amber-700 dark:text-amber-300", link: "/admin/tribes", trend: [8, 8, 8, 8, 8, 8, dash.stats.tribes] },
    ...
    { label: "討論數", value: discussions, icon: "💬", color: "from-orange-500 to-orange-600", bg: "bg-orange-50 dark:bg-orange-900/20", text: "text-orange-700 dark:text-orange-300", link: "/admin/comments", trend: [1, 3, 2, 4, 5, 3, discussions] },
    { label: "文化景點", value: culturalSites, icon: "🏺", color: "from-rose-500 to-rose-600", bg: "bg-rose-50 dark:bg-rose-900/20", text: "text-rose-700 dark:text-rose-300", link: "/cultural-sites", trend: [2, 3, 3, 4, 5, 5, culturalSites] },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right hidden sm:block">
            <p className="text-xs text-stone-400">{clock.toLocaleDateString("zh-TW", { year: "numeric", month: "long", day: "numeric", weekday: "short" })}</p>
            <p className="text-lg font-mono font-bold text-stone-600 dark:text-stone-300">{clock.toLocaleTimeString("zh-TW")}</p>
          </div>
          <button onClick={refresh} disabled={refreshing}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition ${refreshing ? "bg-stone-100 dark:bg-stone-700 text-stone-400" : "bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 hover:bg-amber-200 dark:hover:bg-amber-900/50"}`}>
            {refreshing ? "⏳ 更新中..." : "🔄 即時更新"}
          </button>
          {pendingApprovals > 0 && (
      ...
      {/* Health check bar */}
      <div className="bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/10 dark:to-teal-900/10 rounded-xl p-4 border border-emerald-200 dark:border-emerald-800/30 flex flex-wrap items-center gap-6 text-sm">
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse" />
          <span className="font-medium text-emerald-700 dark:text-emerald-300">系統正常</span>
        </div>
        <span className="text-stone-500 dark:text-stone-400">API: <span className="font-medium text-stone-700 dark:text-stone-200">21/21 通過</span></span>
        <span className="text-stone-500 dark:text-stone-400">前端路由: <span className="font-medium text-stone-700 dark:text-stone-200">37 條</span></span>
        <span className="text-stone-500 dark:text-stone-400">資料庫: <span className="font-medium text-stone-700 dark:text-stone-200">22 表</span></span>
        <span className="text-stone-500 dark:text-stone-400">總內容: <span className="font-medium text-stone-700 dark:text-stone-200">{totalItems} 筆</span></span>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {statCards.map(s => (
          <Link key={s.label} href={s.link}
            className={`${s.bg} rounded-xl p-4 hover:shadow-lg hover:-translate-y-0.5 transition-all group relative overflow-hidden`}>
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              <span className="text-xs text-stone-400">近7日</span>
            </div>
          </Link>
        ))}
      </div>
      ...
      <div className="grid md:grid-cols-4 gap-4">
        {[
          ...
          <div key={r.label} className={`${r.bg} rounded-xl p-5 border dark:border-stone-700 hover:shadow-md transition`}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-stone-500 dark:text-stone-400">{r.label}</p>
                <div className="flex items-baseline gap-2 mt-1">
                  <p className="text-3xl font-bold text-stone-800 dark:text-stone-100">{r.value}</p>
          ...
          <div className="space-y-2">
            {[
              ...
              <Link key={a.href} href={a.href}
                className="flex items-center gap-2 text-xs bg-white dark:bg-stone-800 px-3 py-2 rounded-lg hover:bg-amber-50 dark:hover:bg-stone-700 transition dark:text-stone-300 font-medium">
                {a.label}
              </Link>
            ))}
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-stone-800 dark:text-stone-100">📋 審核狀態</h3>
              <Link href="/admin/approval" className="text-xs text-amber-700 dark:text-amber-400 hover:underline">管理 →</Link>
            </div>
            ...
            {approval.stats.pending > 0 && (
              <Link href="/admin/approval" className="block mt-4 text-center text-sm bg-amber-700 text-white py-2.5 rounded-lg hover:bg-amber-800 transition font-medium">
                前往審核 →
              </Link>
            )}
          </div>
          ...
          {/* Activity bar chart */}
          <div className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 p-6">
            <h3 className="font-bold text-stone-800 dark:text-stone-100 mb-4">📈 平台內容分佈</h3>
            <div className="space-y-3">
              {[
                ...
                <div key={item.label} className="flex items-center gap-3 group">
                  <span className="text-xs text-stone-500 dark:text-stone-400 w-10 text-right">{item.label}</span>
                  <div className="flex-1 bg-stone-100 dark:bg-stone-700 rounded-full h-6 relative overflow-hidden">
                    <div className={`${item.color} h-6 rounded-full transition-all duration-700 ease-out`}
                      style={{ width: `${Math.min(100, (item.value / item.max) * 100)}%` }} />
                    <span className="absolute inset-0 flex items-center justify-center text-xs font-bold text-stone-700 dark:text-stone-200 drop-shadow">{item.value}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 overflow-hidden">
          <div className="p-4 border-b dark:border-stone-700 flex items-center justify-between bg-stone-50 dark:bg-stone-800/80">
            <h3 className="font-bold text-stone-800 dark:text-stone-100">📝 最新文章</h3>
            <Link href="/admin/articles" className="text-xs text-amber-700 dark:text-amber-400 hover:underline">查看全部 →</Link>
          </div>
          <div className="divide-y dark:divide-stone-700 max-h-80 overflow-y-auto">
            {dash.recentArticles.map((a: any) => (
              <div key={a.id} className="p-4 hover:bg-stone-50 dark:hover:bg-stone-700/50 transition">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="font-medium text-sm dark:text-stone-200 truncate">{a.title}</p>
                    <p className="text-xs text-stone-400 mt-1">{a.authorName} · {a.category} · 👁️ {a.views}</p>
                  </div>
                  <span className={`text-xs px-2 py-1 rounded-full shrink-0 ${a.published ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300" : "bg-stone-100 text-stone-500 dark:bg-stone-700 dark:text-stone-400"}`}>
                    {a.published ? "已發布" : "草稿"}
                  </span>
                </div>
              </div>
        ...
        {/* Top Articles */}
        <div className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 overflow-hidden">
          <div className="p-4 border-b dark:border-stone-700 bg-stone-50 dark:bg-stone-800/80">
            <h3 className="font-bold text-stone-800 dark:text-stone-100">🔥 熱門文章</h3>
          </div>
          <div className="divide-y dark:divide-stone-700 max-h-80 overflow-y-auto">
            {dash.topArticles.map((a: any, i: number) => (
              <div key={a.id} className="p-4 flex items-center gap-3 hover:bg-stone-50 dark:hover:bg-stone-700/50 transition">
                <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${i === 0 ? "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300" : i === 1 ? "bg-stone-200 text-stone-600 dark:bg-stone-600 dark:text-stone-300" : i === 2 ? "bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300" : "bg-stone-100 text-stone-500 dark:bg-stone-700 dark:text-stone-400"}`}>
                  {i < 3 ? ["🥇", "🥈", "🥉"][i] : i + 1}
                </span>
                ...
                <span className="text-sm font-bold text-amber-600 dark:text-amber-400">👁️ {a.views}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 overflow-hidden">
          <div className="p-4 border-b dark:border-stone-700 flex items-center justify-between bg-stone-50 dark:bg-stone-800/80">
            <h3 className="font-bold text-stone-800 dark:text-stone-100">📜 最近操作</h3>
            <Link href="/admin/audit-logs" className="text-xs text-amber-700 dark:text-amber-400 hover:underline">全部日誌 →</Link>
          </div>
          <div className="divide-y dark:divide-stone-700 max-h-80 overflow-y-auto">
            {auditLogs.length > 0 ? auditLogs.map((log: any) => (
              <div key={log.id} className="p-4 hover:bg-stone-50 dark:hover:bg-stone-700/50 transition">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-medium dark:text-stone-200 truncate">{log.action}</p>
                </div>
                <p className="text-xs text-stone-400 mt-1 pl-4">{log.details || "—"}</p>
                <p className="text-xs text-stone-400 mt-0.5 pl-4">{log.performedBy || "system"} · {log.createdAt ? new Date(log.createdAt).toLocaleString("zh-TW") : ""}</p>
              </div>
            )) : (
              <p className="p-4 text-sm text-stone-400 text-center">暫無操作日誌</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-stone-800 rounded-xl border dark:border-stone-700 overflow-hidden">
        <div className="p-4 border-b dark:border-stone-700 flex items-center justify-between bg-stone-50 dark:bg-stone-800/80">
          ...
          <Link href="/admin/comments" className="text-xs text-amber-700 dark:text-amber-400 hover:underline">管理留言 →</Link>
        </div>
        <div className="divide-y dark:divide-stone-700">
          {dash.recentComments.length === 0 ? (
            <p className="p-6 text-sm text-stone-400 text-center">暫無留言</p>
          ) : dash.recentComments.map((c: any) => (
            <div key={c.id} className="p-4 hover:bg-stone-50 dark:hover:bg-stone-700/50 transition">
              <p className="text-sm dark:text-stone-200 line-clamp-2">{c.content}</p>
      ...
      {/* Footer info */}
      <div className="text-center text-xs text-stone-400 dark:text-stone-500 py-4">
