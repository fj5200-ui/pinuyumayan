  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          <div key={s.label} className="bg-white dark:bg-[#1a1a1a] rounded-[var(--radius-md)] p-3 border dark:border-[#333] text-center">
            ...
            <p className="text-xl font-bold text-[var(--text-main)] dark:text-gray-100 mt-1">{s.count}</p>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <span className="text-xs text-[var(--text-light)] self-center mr-1">類型：</span>
        {["全部", ...Object.keys(typeColors)].map(t => (
          <button key={t} onClick={() => setTypeFilter(t)}
            className={`px-3 py-1 rounded-full text-xs font-medium transition ${typeFilter === t ? "bg-[var(--red)] text-white" : "bg-white dark:bg-[#1a1a1a] border dark:border-[#333] hover:bg-[var(--cream)] dark:hover:bg-[#333]"}`}>
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        <span className="text-xs text-[var(--text-light)] self-center mr-1">狀態：</span>
        {["全部", "即將舉辦", "進行中", "已結束"].map(s => (
          <button key={s} onClick={() => setStatusFilter(s)}
            className={`px-3 py-1 rounded-full text-xs font-medium transition ${statusFilter === s ? "bg-[var(--red)] text-white" : "bg-white dark:bg-[#1a1a1a] border dark:border-[#333] hover:bg-[var(--cream)] dark:hover:bg-[#333]"}`}>
            {s}
          </button>
        ))}
      </div>
      ...
      {loading ? <GridSkeleton count={6} /> : filteredEvents.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEvents.map((e: any) => {
            const status = getEventStatus(e.startDate, e.endDate);
            return (
              <div key={e.id} className="bg-white dark:bg-[#1a1a1a] rounded-[var(--radius-md)] shadow-sm border border-[var(--border)] dark:border-[#333] hover:shadow-md transition overflow-hidden group">
                {/* Cover gradient */}
                <div className={`h-2 ${e.type === "祭典" ? "bg-gradient-to-r from-red-400 to-[var(--yellow)]" : e.type === "活動" ? "bg-gradient-to-r from-blue-400 to-cyan-400" : e.type === "工作坊" ? "bg-gradient-to-r from-green-400 to-emerald-400" : "bg-gradient-to-r from-purple-400 to-pink-400"}`} />
                ...
                <div className="p-6">
                  ...
                  <h2 className="text-lg font-bold text-[var(--text-main)] dark:text-gray-100 mb-2 group-hover:text-[var(--red)] dark:group-hover:text-[var(--yellow)] transition">{e.title}</h2>
                  <p className="text-[var(--text-soft)] dark:text-[var(--text-light)] text-sm line-clamp-2 mb-4">{e.description}</p>
      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setSelectedEvent(null)}>
          <div className="bg-white dark:bg-[#1a1a1a] rounded-[var(--radius-md)] max-w-lg w-full max-h-[85vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            {/* Header gradient */}
            <div className={`h-3 rounded-t-2xl ${selectedEvent.type === "祭典" ? "bg-gradient-to-r from-red-400 to-[var(--yellow)]" : selectedEvent.type === "活動" ? "bg-gradient-to-r from-blue-400 to-cyan-400" : "bg-gradient-to-r from-green-400 to-emerald-400"}`} />
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setSelectedEvent(null)} className="text-[var(--text-light)] hover:text-[var(--text-soft)] text-xl">✕</button>
              </div>

              <h2 className="text-2xl font-bold text-[var(--text-main)] dark:text-gray-100 mb-4">{selectedEvent.title}</h2>
              ...
              <div className="mt-4 mb-6">
                <h3 className="font-bold text-[var(--text-main)] dark:text-gray-200 mb-2">活動說明</h3>
                <p className="text-[var(--text-soft)] dark:text-[var(--text-light)] leading-relaxed whitespace-pre-line">{selectedEvent.description}</p>
              </div>
              ...
              <button onClick={() => { handleRegister(selectedEvent.id); }}
                ...
                className={`w-full py-3 rounded-[var(--radius-md)] text-sm font-medium transition ${
                  getEventStatus(selectedEvent.startDate, selectedEvent.endDate).label === "已結束" ? "bg-gray-100 dark:bg-[#222] text-[var(--text-light)] cursor-not-allowed" :
