    ...
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      <div className="flex gap-2 mb-8">
        {([["all", "全部", null], ["photo", "📷 照片", "photo"], ["video", "🎬 影片", "video"], ["audio", "🎵 音檔", "audio"]] as [string, string, string | null][]).map(([k, l]) => (
          <button key={k} onClick={() => setFilter(k)}
            ...
            {l}
            <span className={`text-xs px-1.5 py-0.5 rounded-full ${filter === k ? "bg-white/20" : "bg-gray-100 dark:bg-[#222]"}`}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((m: any) => (
            <div key={m.id} onClick={() => setSelected(m)}
              className="bg-white dark:bg-[#1a1a1a] rounded-[var(--radius-md)] shadow-sm overflow-hidden border border-[var(--border)] dark:border-[#333] hover:shadow-lg hover:-translate-y-1 transition-all cursor-pointer group">
              {/* Thumbnail area */}
              <div className="relative bg-gradient-to-br from-gray-100 to-gray-200 dark:from-[#333] dark:to-[#444] h-48 flex items-center justify-center overflow-hidden">
                {m.url && m.type === "photo" ? (
                ...
                {(m.type === "video" || m.type === "audio") && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 group-hover:opacity-100 transition">
                    <div className="w-14 h-14 rounded-full bg-white/90 dark:bg-[#1a1a1a]/90 flex items-center justify-center shadow-lg">
                ...
                <div className="absolute top-3 left-3">
                  <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${typeColors[m.type] || "bg-gray-100 text-[var(--text-soft)]"}`}>
          ...
          {filtered.map((m: any) => (
            <div key={m.id} onClick={() => setSelected(m)}
              ...
              <div className="flex-1 min-w-0">
                ...
                {m.description && <p className="text-[var(--text-soft)] dark:text-[var(--text-light)] text-sm truncate">{m.description}</p>}
              </div>
              <span className={`text-xs font-medium px-2.5 py-1 rounded-full shrink-0 ${typeColors[m.type]}`}>{typeLabel[m.type]}</span>
              <span className="text-xs text-[var(--text-light)] shrink-0">{new Date(m.createdAt).toLocaleDateString("zh-TW")}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {selected && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) setSelected(null); }}>
          <div className="bg-white dark:bg-[#1a1a1a] rounded-[var(--radius-md)] shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            {/* Close */}
            <div className="flex items-center justify-between p-4 border-b dark:border-[#333]">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-bold text-lg dark:text-gray-100">{selected.title}</h2>
              </div>
              <button onClick={() => setSelected(null)} className="text-[var(--text-light)] hover:text-[var(--text-soft)] dark:hover:text-gray-200 text-xl p-1">✕</button>
            </div>
            ...
            <div className="p-6">
              ...
              {selected.type === "photo" && selected.url && (
                <div className="rounded-[var(--radius-md)] overflow-hidden mb-4 bg-gray-100 dark:bg-[#222]">
              ...
              {selected.type === "video" && selected.url && (
                <div className="rounded-[var(--radius-md)] overflow-hidden mb-4 bg-black">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="bg-[var(--cream)] dark:bg-[#222]/50 rounded-lg p-3">
                  <p className="text-[var(--text-light)] text-xs mb-1">上傳日期</p>
                  <p className="font-medium dark:text-gray-200">{new Date(selected.createdAt).toLocaleDateString("zh-TW", { year: "numeric", month: "long", day: "numeric" })}</p>
                </div>
                <div className="bg-[var(--cream)] dark:bg-[#222]/50 rounded-lg p-3">
                  <p className="text-[var(--text-light)] text-xs mb-1">媒體類型</p>
                  <p className="font-medium dark:text-gray-200">{typeIcon[selected.type]} {typeLabel[selected.type]}</p>
                </div>
                {selected.tribeName && (
                  <div className="bg-[var(--cream)] dark:bg-[#222]/50 rounded-lg p-3">
