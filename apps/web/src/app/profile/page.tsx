...
export default function ProfilePage() {
  ...
  const [tab, setTab] = useState<"info" | "bookmarks" | "follows" | "password">("info");
    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="bg-white dark:bg-stone-800 rounded-2xl shadow-sm p-8 border dark:border-stone-700 mb-6">
        ...
        <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t dark:border-stone-700">
          ...
          <div className="text-center"><p className="text-2xl font-bold text-amber-700 dark:text-amber-400">0</p><p className="text-sm text-stone-500">學習進度</p></div>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex flex-wrap gap-2 mb-4">
        {([["info", "📋 基本資料"], ["bookmarks", `📚 收藏 (${bookmarks.length})`], ["follows", `🏘️ 追蹤 (${follows.length})`], ["password", "🔒 修改密碼"]] as const).map(([t, label]) => (
